...
const BotaoPanicoScreen = ({ navigation }) => {
  ...
  const tocarAlarme = async () => {
    try {
      ...
      // Adicionar o som à lista de alarmes
      setAlarms((prevAlarms) => [...prevAlarms, sound]);
  ...
  const pararAlarme = async () => {
    // Parar todos os alarmes na lista
    for (const sound of alarms) {
      ...
      >
        <View style={styles.container3}>
          <View style={styles.popUp}>
            ...
            <Text style={styles.text}> <Text style={styles.negrito}> - Usuário Lipa's: </Text> Já o usuário Lipa's é o tipo de contato que tem conta em nosso aplicativo, sendo assim, além de receber a mensagem de socorro, com ele também vai ter o compartilhamento da localização, tanto a dele quanto a sua. </Text>

            <Text style={styles.subtitulo2}>2. Botão de Pânico: </Text>
            <Text style={styles.text}> O aplicativo possui um botão de pânico de fácil acesso. </Text>
