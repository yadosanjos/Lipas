  ...
  const solicitarPermissaoLocalizacao = async () => {
    return new Promise((resolve) => {
      Alert.alert(
        ...
        `Deseja permitir o compartilhamento de localização com ${nomeContato}?`,
        [
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!permissaoConcedida) {
        Alert.alert('Permissão Negada', 'Compartilhamento de localização não autorizado.');
        return;
      }


      // Adiciona o contato no Firebase com flag de permissão
      await addDoc(contactsRef, {
        ...
        outroCelular,
        type: 'lipas',
        compartilhamentoAutorizado: permissaoConcedida,
      });

      Alert.alert('Sucesso', 'Contato de emergência cadastrado e compartilhamento autorizado!');
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView>
        <Text style={styles.subheader}>
          Cadastre uma pessoa próxima e de confiança para ser seu contato de emergência. Diferente do usuário comum, 
          usuário Lipa’s é o seu contato que tem conta em nosso aplicativo.
        </Text>
        ...
        <View style={styles.inputContainer}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Celular com 55 e DDD*"
